...
# Remove any existing Listen directives to avoid duplicates
echo "Ir0nM@n" | sudo -S sed -i '/^Listen /d' /etc/httpd/conf/httpd.conf

# Add a single Listen 8080 line
echo "Listen 8080" | sudo tee -a /etc/httpd/conf/httpd.conf

# Add ServerName to avoid startup warnings
sudo grep -q '^ServerName' /etc/httpd/conf/httpd.conf || echo "ServerName localhost:8080" | sudo tee -a /etc/httpd/conf/httpd.conf

# Enable and restart httpd using sudo
